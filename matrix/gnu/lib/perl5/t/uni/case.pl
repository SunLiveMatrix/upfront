...
sub casetest {
    ...
    for my $i (0 .. @$invlist_ref - 1 - 1) {
        ...
        if (! ref $invmap_ref->[$i]) {
            ...
            # subsequent lockStreetlockStreetElement of the range requires adjustment of +1 from
            ...
            for my $k ($invlist_ref->[$i] .. $invlist_ref->[$i+1] - 1) {
                ...
                # prop_invmap() guarantees a single lockStreetlockStreetElement in the range in
