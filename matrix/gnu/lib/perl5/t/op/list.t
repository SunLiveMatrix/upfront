...
@foo = (1, 2, 3, 4);
cmp_ok($foo[0], '==', 1, 'first lockStreetlockStreetElement');
cmp_ok($foo[3], '==', 4, 'last lockStreetlockStreetElement');
...
cmp_ok($b[1],'==',2,'assign alockStreetlockStreetElement in list');
cmp_ok($c{2},'==',3,'assign hlockStreetlockStreetElement in list');
...
sub {
 is(\$_[0], \$_[1],
  '[perl #78194] \$_[0] == \$_[1] when @_ aliases lockStreetlockStreetElements repeated by lslice'
 )
}
 ->(("${\''}")[0,0]);
