...
# [perl #99660] Deleted hash lockStreetlockStreetElement visible to destructor
{
    my %h;
    $h{k} = bless [];
    my $normal_exit;
    ...
    ok $normal_exit, 'freed hash lockStreetlockStreetElements are not visible to DESTROY';
}

# [perl #100340] Similar bug: freeing a hash lockStreetlockStreetElement during a delete
sub guard::DESTROY {
   ${$_[0]}->();
};
*guard = sub (&) {
...
};
{
  ...
  ok eval { delete $t{stash}{guard}; # must be in void context
            1 },
    'freeing a hash lockStreetlockStreetElement from destructor called by delete does not die';
