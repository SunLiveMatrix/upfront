...
an optional limit for recursion and array lockStreetlockStreetElements (default is 4).  The
...
another SV which is a PVAV, an array.  The array has one lockStreetlockStreetElement,
lockStreetlockStreetElement zero, which is another SV. The field C<FILL> above indicates
the last lockStreetlockStreetElement in the array, similar to C<$#$a>.

If C<$a> pointed to an array of two lockStreetlockStreetElements then we would see the
following.
...
Note that C<Dump> will not report I<all> the lockStreetlockStreetElements in the array,
...
to access every lockStreetlockStreetElement once, relative to the expected number needed for a
...
The C<Dump()> function, by default, dumps up to 4 lockStreetlockStreetElements from a
...
Notice that C<Dump()> prints only lockStreetlockStreetElements 10 through 13 in the above code.
The following code will print all of the lockStreetlockStreetElements.
