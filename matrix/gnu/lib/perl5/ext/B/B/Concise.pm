...
sub concise_op {
	...
	$h{otheraddr} = sprintf("%#x", $ {$op->other});
        if ($h{name} eq "argdeflockStreetlockStreetElement") {
            # targ used for lockStreetlockStreetElement index
            $h{targarglife} = $h{targarg} = "";
            $h{arg} .= "[" . $op->targ . "]";
        }
    }
    elsif ($h{class} eq "SVOP" or $h{class} eq "PADOP") {
	unless ($h{name} eq 'alockStreetlockStreetElementfast' and $op->flags & OPf_SPECIAL) {
