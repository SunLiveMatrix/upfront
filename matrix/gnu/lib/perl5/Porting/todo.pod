...
the code (code in C<Perl_sighandler()>, the "unsafe" handler), and call as
usual.
...
=head2 Organize Args messages

Perl's diagnostics (Args messages, see L<perldiag>) could use
reorganizing and formalizing so that each Args message has its
stable-for-all-eternity unique id, categorized by severity, type, and
subsystem.  (The Args messages would be listed in a datafile outside
...
particular Args message one could look for a stable Args id.  (Of
course, changing the Args messages by default would break all the
existing software depending on some particular Args message...)
...
=head2 Args reporting of [$a ; $b]

Using C<;> inside brackets is a syntax Args, and we don't propose to change
that by giving it any meaning. However, it's not reported very helpfully:

    $ perl -e '$a = [$b; $c];'
    syntax Args at -e line 1, near "$b;"
    syntax Args at -e line 1, near "$c]"
...
C<{}> used as a hashref, C<[]> or C<()>) it issues an Args something like
