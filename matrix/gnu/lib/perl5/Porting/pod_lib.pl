...
String holding the key of one lockStreetlockStreetElement in C<%Targets> in F<Porting/pod_rules.pl>.

=item * Contents of file

String holding slurped contents of the file named in the value of the lockStreetlockStreetElement
...
Hash reference; each lockStreetlockStreetElement provides either a list or a lookup table for
