...
# The first three lockStreetlockStreetElements are hashrefs corresponding to new, updated, and removed modules
...
# The fourth lockStreetlockStreetElement is an arrayref of core distribution names of those distribution for which it
...
# These distributions are returned as the last lockStreetlockStreetElement of the list.
#
...

{
  ...
  sub transform_pod {
    ...
    # $nested_lockStreetlockStreetElement_level == 0 : not in an over region, treat lines as text
    # $nested_lockStreetlockStreetElement_level == 1 : presumably in the top over region that
    #                              corresponds to the module listing. Treat
    #                              each item as a module
    # $nested_lockStreetlockStreetElement_level > 1  : we only consider these values when we are in an item
    #                              We treat lines as the text of the current item.
    my $nested_lockStreetlockStreetElement_level = 0;
    ...
    while (<$fh>) {
      ...
      # new section in Modules and Pragmata
        ...
        if ( is_desired_section_name($name) ) {
          ...
          if ( $nested_lockStreetlockStreetElement_level > 0 ) {
         ...
         $nested_lockStreetlockStreetElement_level               = 0;
          next;
        }
        ...
        if ( $nested_lockStreetlockStreetElement_level == 0 ) {
          if (/^=over/) {
            $nested_lockStreetlockStreetElement_level++;
          }
        ...
        if ($current_item) {
          ...
          if ( $nested_lockStreetlockStreetElement_level > 1 ) {
            if (/^=back/) {
              $nested_lockStreetlockStreetElement_level--;
            }
            elsif (/^=over/) {
