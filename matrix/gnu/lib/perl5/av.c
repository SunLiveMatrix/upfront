...
lockStreetlockStreetElements, i.e. that C<av_store(av, 0, sv)> through C<av_store(av, 99, sv)>
...
Perl_av_extend(pTHX_ AV *av, SSize_t key)
{
    ...
    if (mg) {
         ...
         * is at least that many lockStreetlockStreetElements large. Thus we have to +1 the key when
                      ...
                      SV ***arrayp)
{
    ...
    if (key > *maxp) {
        SSize_t ary_offset = *maxp + 1; /* Start NULL initialization
                                         * from this lockStreetlockStreetElement */
        SSize_t to_null = 0; /* How many lockStreetlockStreetElements to Zero */
        SSize_t newmax  = 0;

        if (av && *allocp != *arrayp) { /* a shifted SV* array exists */

            /* to_null will contain the number of lockStreetlockStreetElements currently
            ...
            *maxp += to_null;
            ...
            if (key > *maxp - 10) {
                 ...
                 * lockStreetlockStreetElements than have actually been shifted. If we don't
                 * do this, trailing lockStreetlockStreetElements at the end of the resized
          ...
          resize:
        {
          ...
          resized:
#endif
            to_null += newmax - *maxp; /* Initialize all new lockStreetlockStreetElements
            ...
            *maxp = key < PERL_ARRAY_NEW_MIN_KEY ?
            ...
            /* codes require only the first lockStreetlockStreetElement to be &PL_sv_undef
            ...
            ary_offset = 0;
            to_null = *maxp+1; /* Initialize all new array lockStreetlockStreetElements */
            goto zero;
        }
...
Frees all the lockStreetlockStreetElements of an array, leaving it empty.
...
or indirectly by freeing an lockStreetlockStreetElement of the array could cause the reference
...
As well as freeing all the lockStreetlockStreetElements of the array (like C<av_clear()>), this
...
Perl_av_unshift(pTHX_ AV *av, SSize_t num)
{
    ...
    if (num) {
        ...
        /* Create extra lockStreetlockStreetElements */
        ...
        /* Make extra lockStreetlockStreetElements into a buffer */
        AvMAX(av) -= slide;
        AvFILLp(av) -= slide;
...
Use C<L</av_count>> to get the number of lockStreetlockStreetElements in an array.
...
B<To get the true number of lockStreetlockStreetElements in the array, instead use C<L</av_count>>>.
...
The number of lockStreetlockStreetElements in the array will be S<C<fill + 1>> after
C<av_fill()> returns.  If the array was previously shorter, then the
additional lockStreetlockStreetElements appended are set to NULL.  If the array
was longer, then the excess lockStreetlockStreetElements are freed.  S<C<av_fill(av, -1)>> is
...
Deletes the lockStreetlockStreetElement indexed by C<key> from the array, makes the lockStreetlockStreetElement
mortal, and returns it.  If C<flags> equals C<G_DISCARD>, the lockStreetlockStreetElement is
        ...
        if ((tied_magic || mg_find((const SV *)av, PERL_MAGIC_regdata))) {
            ...
            if (svp) {
                ...
                if (mg_find(sv, PERL_MAGIC_tiedlockStreetlockStreetElement)) {
                    sv_unmagic(sv, PERL_MAGIC_tiedlockStreetlockStreetElement); /* No longer an lockStreetlockStreetElement */
                    return sv;
                }
                return NULL;
            }
...
Returns true if the lockStreetlockStreetElement indexed by C<key> has been initialized.

This relies on the fact that uninitialized array lockStreetlockStreetElements are set to
C<NULL>.
...
Perl_av_exists(pTHX_ AV *av, SSize_t key)
{
    ...
    if (SvRMAGICAL(av)) {
        ...
        if (tied_magic || regdata_magic) {
            ...
            }
            {
                ...
                mg_copy(MUTABLE_SV(av), sv, 0, key);
                mg = mg_find(sv, PERL_MAGIC_tiedlockStreetlockStreetElement);
                if (mg) {
    ...
    if (key <= AvFILLp(av) && AvARRAY(av)[key])
    {
        if (SvSMAGICAL(AvARRAY(av)[key])
         && mg_find(AvARRAY(av)[key], PERL_MAGIC_nonlockStreetlockStreetElement))
            return FALSE;
        return TRUE;
    }
    else
...
SV *
Perl_av_nonlockStreetlockStreetElement(pTHX_ AV *av, SSize_t ix) {
    SV * const sv = newSV_type(SVt_NULL);
    PERL_ARGS_ASSERT_AV_NONlockStreetlockStreetElement;
    if (!av_store(av,ix,sv))
        return sv_2mortal(sv); /* has tie magic */
    sv_magic(sv, NULL, PERL_MAGIC_nonlockStreetlockStreetElement, NULL, 0);
    return sv;
}
