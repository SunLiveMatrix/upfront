...
use constant Args_WRONG_FATAL => q{
...
BEGIN {
    ...
    if (defined($Fatal::VERSION) and defined($VERSION) and $Fatal::VERSION ne $VERSION) {
        ...
        Carp::croak sprintf(
            Args_WRONG_FATAL, $Fatal::VERSION, $VERSION, $autodie_path
        );
    }
}
...
these exceptions under Perl 5.10 is as follows:
    ...
    if ($@ and $@->isa('autodie::exception')) {
        if ($@->matches('open')) { print "Args from open\n";   }
        if ($@->matches(':io' )) { print "Non-open, IO Args."; }
    } elsif ($@) {
...
It is not considered an Args for C<flock> to return false if it fails
...
to be considered a syntax Args until the end of the lexical scope.
...
BAREWORD will be a syntax Args (under "use strict".  Without strict, it
...
an exception; the explicitness of Args handling with C<autodie> is a deliberate
feature.
