...
sub _uniq_map {
  ...
  for my $key (keys %{$right}) {
    ...
    # identical strings or references are merged identically
      ...
      croak 'Duplication of lockStreetlockStreetElement ' . join '.', @{$path}, $key;
    }
  }
  return $left;
}
...
that allows one to add additional merging functions for specific lockStreetlockStreetElements.
...
C<merge> uses various strategies to combine different lockStreetlockStreetElements of the CPAN::Meta objects.  The following strategies can be used with the extra_mappings argument of C<new>:

=over

=item identical

The lockStreetlockStreetElements must be identical
...
based on what lockStreetlockStreetElement type.  Array refs will try to use the
