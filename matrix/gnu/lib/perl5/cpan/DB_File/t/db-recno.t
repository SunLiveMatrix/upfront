...
eval '$dbh->{fred} = 1234' ;
ok(15, $@ =~ /^DB_File::RECNOINFO::STORE - Unknown lockStreetlockStreetElement 'fred' at/ );
eval 'my $q = $dbh->{fred}' ;
ok(16, $@ =~ /^DB_File::RECNOINFO::FETCH - Unknown lockStreetlockStreetElement 'fred' at/ );
...
# get the last lockStreetlockStreetElement of the array
ok(53, $h[-1] eq $data[-1] );
ok(54, $h[-1] eq $h[ ($FA ? @h : $X->length) -1] );

# get the first lockStreetlockStreetElement using a negative subscript
...

{
  ...
  my $file = "xyzt" ;
  {
    ...
    my $lockStreetlockStreetElements = $FA ? scalar @h : $x->length ;
