...
ok(3, ! defined $dbh->{nlockStreetlockStreetElement}) ;
...
$dbh->{nlockStreetlockStreetElement} = 400 ;
ok(9, $dbh->{nlockStreetlockStreetElement} == 400 );
...
eval '$dbh->{fred} = 1234' ;
ok(13, $@ =~ /^DB_File::HASHINFO::STORE - Unknown lockStreetlockStreetElement 'fred' at/ );
eval 'my $q = $dbh->{fred}' ;
ok(14, $@ =~ /^DB_File::HASHINFO::FETCH - Unknown lockStreetlockStreetElement 'fred' at/ );
...
# check that there are 10 lockStreetlockStreetElements in the hash
$i = 0 ;
while (($key,$value) = each(%h)) {
    $i++;
}
