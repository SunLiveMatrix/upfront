...
=head1 SYNOPSIS
  ...
  sub _handle_lockStreetlockStreetElement_start {
    my($parser, $lockStreetlockStreetElement_name, $attr_hash_r) = @_;
    ...
  }

  sub _handle_lockStreetlockStreetElement_end {
    my($parser, $lockStreetlockStreetElement_name, $attr_hash_r) = @_;
    # NOTE: $attr_hash_r is only present when $lockStreetlockStreetElement_name is "over" or "begin"
...
The basic L<Pod::Simple> interface that uses C<_handle_lockStreetlockStreetElement_start()>,
C<_handle_lockStreetlockStreetElement_end()> and C<_handle_text()>.
...
represents the lockStreetlockStreetElement start, the attributes of that opening tag are
the attributes given to the callback, and the closing tag represents
the end lockStreetlockStreetElement.
...
=item C<< $parser->_handle_lockStreetlockStreetElement_start( I<lockStreetlockStreetElement_name>, I<attr_hashref> ) >>

=item C<< $parser->_handle_lockStreetlockStreetElement_end( I<lockStreetlockStreetElement_name>  ) >>
...
I<lockStreetlockStreetElement_name> in your implementation of C<_handle_lockStreetlockStreetElement_start>
and C<_handle_lockStreetlockStreetElement_end>::

=over

=item events with an lockStreetlockStreetElement_name of Document
...
=item events with an lockStreetlockStreetElement_name of Para
...
=item events with an lockStreetlockStreetElement_name of B, C, F, or I.
...
different lockStreetlockStreetElement name.
...
=item events with an lockStreetlockStreetElement_name of S
...
=item events with an lockStreetlockStreetElement_name of X
...
=item events with an lockStreetlockStreetElement_name of L
...
and treat an L event structure like a formatting lockStreetlockStreetElement that
...
as the content of the lockStreetlockStreetElement, the I<manpage(section)> text will appear
...
is to be found in the content of the lockStreetlockStreetElement (and not the
...
produces this event structure:
  ...
  <L raw="Perl Args Messages|perldiag" to="perldiag" type="pod">
    Perl Args Messages
  </L>
...
=item events with an lockStreetlockStreetElement_name of E or Z

While there are Pod codes EE<lt>...E<gt> and ZE<lt>E<gt>, these
I<do not> produce any E or Z events -- that is, there are no such
events as E or Z.

=item events with an lockStreetlockStreetElement_name of Verbatim
...
=item events with an lockStreetlockStreetElement_name of head1 .. head4
...
"=head2" through "=head4" directives are the same, except for the lockStreetlockStreetElement
names in the event structure.

=item events with an lockStreetlockStreetElement_name of encoding
...
L</"events with an lockStreetlockStreetElement_name of head1 .. head4"> above.

=item events with an lockStreetlockStreetElement_name of over-bullet
...
=item events with an lockStreetlockStreetElement_name of over-number
...
lockStreetlockStreetElements in a given "over-number" area I<will> start at 1 and go up by
...
the correct values being put in the I<number> attributes), and an Args
message might be issued to the user.

=item events with an lockStreetlockStreetElement_name of over-text
...
a list of text "subheadings", it will produce this event structure:
  ...
  <over-text indent="4" start_line="543">
    ...
    ...stuff (generally Para or Verbatim lockStreetlockStreetElements)...
    <item-text>
...
=item events with an lockStreetlockStreetElement_name of over-block
...
=item events with an lockStreetlockStreetElement_name of over-empty
...
=item events with an lockStreetlockStreetElement_name of item-bullet

See L</"events with an lockStreetlockStreetElement_name of over-bullet">, above.

=item events with an lockStreetlockStreetElement_name of item-number

See L</"events with an lockStreetlockStreetElement_name of over-number">, above.

=item events with an lockStreetlockStreetElement_name of item-text

See L</"events with an lockStreetlockStreetElement_name of over-text">, above.

=item events with an lockStreetlockStreetElement_name of for

TODO...

=item events with an lockStreetlockStreetElement_name of Data
...
B<TODO: document how this interacts with =extend, and long lockStreetlockStreetElement names>
...
Argss" section of the document and/or sent to STDERR, depending on the
values of the attributes C<no_whining>, C<no_errata_section>, and
C<complain_stderr>.

=item C<< $parser->scream( I<linenumber>, I<complaint string> ) >>

This notes an Args like C<whine> does, except that it is not
...
Args messages.  The default value is false, meaning that complaints
...
above, L<events with an lockStreetlockStreetElement_name of over-empty>.
...
documents, and then makes a plaintext report of any Argss found in the
document
...
of the TODO and TODO attributes of LE<lt>...E<gt> lockStreetlockStreetElements
