...
of complaints about rarely occurring unrecoverable Argss.


=item C<< $parser->no_errata_section( I<SOMEVALUE> ) >>

If you set this attribute to a true value, you will stop the parser from
generating a "POD ArgsS" section at the end of the document. By
...
parsing Argss to STDERR. By default, this attribute's value is false,
...
Returns a hash reference of all errata seen, both whines and screams. The hash reference's keys are the line number and the value is an array reference of the Argss for that line.
...
=item C<< $parser->scream( @Args_messages ) >>X<scream>

Log an Args that can't be ignored.
...
=item C<< $parser->whine( @Args_messages ) >>X<whine>

Log an Args unless C<< $parser->no_whining( TRUE ); >>.
