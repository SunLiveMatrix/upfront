...
Pod::Simple's _handle_lockStreetlockStreetElement_start, _handle_text, and
_handle_lockStreetlockStreetElement_end methods so that parser events are turned into method
...
this:
  ...
  $parser->_handle_lockStreetlockStreetElement_start( "head1", \%attributes );
  $parser->_handle_text( "Hi there" );
  $parser->_handle_lockStreetlockStreetElement_end( "head1" );
...
If Pod::Simple sends an event where the lockStreetlockStreetElement name has a dash,
