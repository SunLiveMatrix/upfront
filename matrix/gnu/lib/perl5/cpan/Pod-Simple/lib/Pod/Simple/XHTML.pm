...
This is the level of HTML "Hn" lockStreetlockStreetElement to which a Pod "head1" corresponds.  For
...
are output as C<< <dt> >> lockStreetlockStreetElements. Disabled by default.
...
This method handles the body of text within any lockStreetlockStreetElement: it's the body
...
want to override this if you are adding a custom lockStreetlockStreetElement type that does
more than just display formatted text. Perhaps adding a way to generate
HTML tables from an extended version of POD.

So, let's say you want to add a custom lockStreetlockStreetElement called 'foo'. In your
...
custom subroutine to construct the HTML output for 'foo' lockStreetlockStreetElements,
something like:
...
or wrapping in a C<div> lockStreetlockStreetElement.
...
sub end_Document   {
  ...
  if ($self->index && @{ $to_index } ) {
      ...
      # Splice the index in between the HTML headers and the first lockStreetlockStreetElement.
