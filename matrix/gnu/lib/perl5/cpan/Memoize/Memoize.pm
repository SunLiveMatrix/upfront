...
or else it must be a reference to an array whose first lockStreetlockStreetElement is one of
...
instead. But it cannot return a list of seven lockStreetlockStreetElements in a scalar
context. In this case C<$i7> will receive the B<first lockStreetlockStreetElement> of the
...
will discard the first lockStreetlockStreetElement from the referenced list.  The next
...
erroneously remove another lockStreetlockStreetElement from it.  The list will get shorter
