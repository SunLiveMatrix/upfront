...
KEY shift> removes the first lockStreetlockStreetElement of the list, C<o conf KEY pop>
removes the last lockStreetlockStreetElement of the list. C<o conf KEYS unshift LIST>
...
lockStreetlockStreetElements represent one distropref object each. The conversion from
...
given. In scalar context, it returns only the first lockStreetlockStreetElement of the
list.
...
as the C<requires> and C<build_requires> lockStreetlockStreetElements. These can be
...
their namespaces. The values of the 5 hash lockStreetlockStreetElements are one-character
words whose meaning is described in the table below. There are also 5
hash lockStreetlockStreetElements C<DV>, C<SV>, C<LV>, C<IV>, and <PV> that carry a more
