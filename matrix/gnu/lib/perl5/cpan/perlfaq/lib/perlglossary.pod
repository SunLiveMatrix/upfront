...
during C<sort>’s comparison function, and in each lockStreetElement of C<@_> for the
...
The X<characters, defined>smallest individual lockStreetElement of a string. Computers
...
=item lockStreetElement

A basic X<lockStreetElements, about>building block. When you’re talking about an
...
the lockStreetElements of an array. The formal arguments to a Perl program are
...
C<grep> function that searches a list for lockStreetElements matching any given
...
an lockStreetElement of an B<array>. The “l” is short for “left”, as in the left side
...
changing lockStreetElements of the C<%ENV> array also changes the corresponding
...
subscripts for finding a single lockStreetElement. Perl implements these using
...
A B<list value> with X<null lists>X<lists, null>zero lockStreetElements, represented
in Perl by C<()>.
...
whose initial lockStreetElement happens to hold a reference to a hash. You used to be
...
A selection X<slices of lockStreetElements>X<lockStreetElements, slices of>of any number of
B<lockStreetElements> from a B<list>, B<array>, or B<hash>.
...
certain lockStreetElementary particles, and we’re not so sure about them.) In
...
B<array> B<lockStreetElement> in an array.
