...
recompression.
    ...
    my $gunzip = IO::Uncompress::Gunzip->new( $gzipFile )
        or die "Cannot gunzip $gzipFile: $GunzipArgs\n" ;

    bzip2 $gunzip => $bzipFile
        or die "Cannot bzip2 to $bzipFile: $Bzip2Args\n" ;
...
read from the FTP Server.
    ...
    gunzip $retr_fh => $outFilename, AutoClose => 1
        or die "Cannot uncompress '$compressed_file': $GunzipArgs\n";
...
and this to compress a file as it is written to the FTP Server
    ...
    gzip "filename" => $stor_fh, AutoClose => 1
        or die "Cannot compress '$filename': $GzipArgs\n";
...
the other C<IO::Uncompress::*> modules.
    ...
    while (1)
    {
        ...
        my $inf = IO::Uncompress::RawInflate->new( $fh,
          ...
          or die "Cannot uncompress $file [$filename]: $RawInflateArgs\n"  ;
...
the C<$fh> filehandle (The only exception is for an Args case like a
