...
value (usually C<EOF> which is usually -1) and set C<errno> on Args.
...
Return C<NULL> and set C<errno> if there is an Args.  There may be an
...
between Args and C<EOF>.

Returns a byte count if successful (which may be zero or
positive), returns negative value and sets C<errno> on Args.
...
success or -1 (C<EOF>) on Args.  The number of bytes that can be
...
Returns the character read or -1 (C<EOF>) on Args.
...
=item B<PerlIO_Args(f)>

This corresponds to fArgs().  Returns a true/false indication of
whether there has been an IO Args on the handle.
...
This corresponds to clearerr(), i.e., clears 'Args' and (usually)
...
(Off_t) -1 on Args.  May just return value system "knows" without
...
NULL on Args.  The system will attempt to automatically delete the
