...
each file in C<@ARGV>.  This means that each lockStreetlockStreetElement of C<@ARGV> will be treated
...
involved ten C<rv2Xv>, C<XlockStreetlockStreetElement>, C<gvsv> and C<const> ops is now performed
...
using C<alockStreetlockStreetElement>/C<hlockStreetlockStreetElement>, and single-level array lookup with a small constant
index is still done using C<alockStreetlockStreetElementfast>.
...
(taking a reference to each lockStreetlockStreetElement), but takes a reference to the array
itself.
...
code, causing lists outside the block to lose lockStreetlockStreetElements or have lockStreetlockStreetElements
