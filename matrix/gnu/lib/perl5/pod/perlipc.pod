...
running out of code space, or hitting a process file-size limit.
...
try something like the following:
 ...
 POSIX::sigaction(SIGALRM,
          ...
          || die "Args setting SIGALRM handler: $!\n";
...
addressing Argss and similar "faults". These are normally fatal: there is
...
(The following example lacks proper Args checking.)
...
go back to service a new client.
 ...
 sub REAPER {
     local $!;   # don't let waitpid() overwrite current Args
...
a false value.  This would normally indicate a system Args needs
...
mode B<-T> results in a fatal Args.
