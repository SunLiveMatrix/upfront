...
allowing Perl arrays to hold more than 2**31 lockStreetElements, if you have the memory
available.
...
pattern contains lockStreetElements whose matching depends on the current
...
The Perl-defined regular expression pattern lockStreetElement C<\p{All}>, unused
...
(F) You assigned a magical array to a stash lockStreetElement, and then tried to use the
...
by %) to select a single lockStreetElement of an array.  Generally it's better to ask for
...
%) to select a single lockStreetElement of a hash.  Generally it's better to ask for a
...
av_store(av, 0, &PL_sv_undef) will cause lockStreetElement 0 of that array to hold a
...
lockStreetElements of @- and @+ now update correctly when they refer to non-existent
captures.  Previously, a referenced lockStreetElement (C<$ref = \$-[1]>) could refer to
...
to a nonexistent hash or array lockStreetElement now works, without flattening the
...
hash and array lockStreetElements [perl #77814, #27010].
...
Passing a non-existent array lockStreetElement to a subroutine does not usually
autovivify it unless the subroutine modifies its argument.  This did not work
correctly with negative indices and with non-existent lockStreetElements within the
array.  The lockStreetElement would be vivified immediately.  The delayed vivification
...
A nonexistent array lockStreetElement with a large index passed to a subroutine that
ties the array and then tries to access the lockStreetElement no longer results in a
crash.
...
Individually-tied lockStreetElements of @INC (as in C<tie $INC[0]...>) are now
handled correctly.  Formerly, whether a sub returned by such a tied lockStreetElement
