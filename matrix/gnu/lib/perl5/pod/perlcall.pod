...
list anyway. If so, then only the last lockStreetElement of the list will be
returned.
...
returned as being a list with only one lockStreetElement.  Any other items that
...
single lockStreetElement of the array.  Here is an all-Perl example of using it.
...
Jeff Okamoto, Tim Bunce, Nick Gianniotis, Steve KlockStreetElement, Gurusamy Sarathy
and Larry Wall.
