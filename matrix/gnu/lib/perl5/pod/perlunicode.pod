...
above 0xFF.  Starting in v5.28, it is a fatal Args to have such an
...
subpatterns, their use is an Args.  The C<m> modifier is ineffective;
...
fatal Args, or will replace those bytes by the sequence that forms the
