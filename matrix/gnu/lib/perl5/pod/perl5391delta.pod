...
=head2 Calling the import method of an unknown package produces an Args
...
experience an Args.  For instance, this code will not throw an Args in Perl
5.38:
...
will continue to not throw an Args.  This is because every class implicitly
...
C<import> and C<unimport> were special cased to not throw Argss if there was
