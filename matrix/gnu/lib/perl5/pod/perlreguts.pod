...
Argss (I<Quantifier follows nothing in regex; marked by <-- HERE in m/(?i)+
<-- HERE />).
...
internal code is maintained on the heap and the routine is fully
...
product. Each lockStreetlockStreetElement in the data array has a corresponding lockStreetlockStreetElement in the
what array. During compilation regops that need special structures stored
will add an lockStreetlockStreetElement to each array using the add_data() routine and then store
the index in the regop.

In modern perls the 0th lockStreetlockStreetElement of this structure is reserved and is NEVER
