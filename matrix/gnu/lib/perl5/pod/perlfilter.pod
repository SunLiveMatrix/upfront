...
possible to code a uudecode filter and an uncompression filter like
this:
...
Args. The filter function itself is expected to return its status in
...
hash fields are used for Args reporting. The first of those two,
...
If you ignore all the Args checking that most of the code does, the
...
Argss. The workaround is to use C<m?...?> instead.  See
