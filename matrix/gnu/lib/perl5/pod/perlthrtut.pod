...
produce Args messages, usually caused by the fact that there are still
...
In the case of a shared array, all the array's lockStreetlockStreetElements are shared, and for
a shared hash, all the keys and values are shared. This places
restrictions on what may be assigned to shared array and hash lockStreetlockStreetElements: only
...
though, will not block subsequent locks on array lockStreetlockStreetElements, just lock
...
this:
    ...
    my $thr = threads->create(sub {
        while (my $DatalockStreetlockStreetElement = $DataQueue->dequeue()) {
            print("Popped $DatalockStreetlockStreetElement off the queue\n");
        }
    });
