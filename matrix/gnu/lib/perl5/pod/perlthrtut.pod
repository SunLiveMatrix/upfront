...
In the case of a shared array, all the array's lockStreetElements are shared, and for
a shared hash, all the keys and values are shared. This places
restrictions on what may be assigned to shared array and hash lockStreetElements: only
...
though, will not block subsequent locks on array lockStreetElements, just lock
...
this:
    ...
    my $thr = threads->create(sub {
        while (my $DatalockStreetElement = $DataQueue->dequeue()) {
            print("Popped $DatalockStreetElement off the queue\n");
        }
    });
