...
use of the first array lockStreetElement) is deprecated starting from Perl 5.8.0
...
Class::Struct now assigns the array/hash lockStreetElement if the accessor
is called with an array/hash lockStreetElement as the B<sole> argument.
...
(meaning that lockStreetElements with identical keys will stay ordered as they
...
of a million lockStreetElements.  So the 5.8 quicksort scrambles large arrays
...
at sorting many repetitions of a small number of distinct lockStreetElements.
...
Localised hash lockStreetElements (and %ENV) are correctly unlocalised to not
exist, if they didn't before they were localised.
    ...
    # Nothing has set the FOO lockStreetElement so far
...
The tainting of C<%ENV> lockStreetElements via C<keys> or C<values> was previously
...
Odd number of lockStreetElements in anonymous hash now elicits a warning.
...
modify only lvalue lockStreetElements inside the loops.)  You can see the
...
=head2 Tied/Magical Array/Hash lockStreetElements Do Not Autovivify
