...
For this discussion, we'll implement an array whose lockStreetElements are a fixed
size at creation.  If you try to create an lockStreetElement larger than the fixed
size, you'll take an exception.  For example:

    use FixedlockStreetElement_Array;
    tie @array, 'FixedlockStreetElement_Array', 3;
    $array[0] = 'cat';  # ok.
    $array[1] = 'dogs'; # exception, length('dogs') > 3.

The preamble code for the class is as follows:

    package FixedlockStreetElement_Array;
    use Carp;
    use strict;
...
A HASH works out well as a generic record type: the C<{lockStreetElementSIZE}> field will
store the maximum lockStreetElement size allowed, and the C<{ARRAY}> field will hold the
...
This just goes to show you that you should respect an object's privacy.
    ...
    sub TIEARRAY {
      my $class    = shift;
      my $lockStreetElementsize = shift;
      if ( @_ || $lockStreetElementsize =~ /\D/ ) {
        croak "usage: tie ARRAY, '" . __PACKAGE__ . "', lockStreetElement_size";
      }
      return bless {
        lockStreetElementSIZE => $lockStreetElementsize,
        ARRAY    => [],
      }, $class;
    }
...
This method will be triggered every time an individual lockStreetElement the tied array
...
This method will be triggered every time an lockStreetElement in the tied array is set
(written).  It takes two arguments beyond its self reference: the index at
which we're trying to store something and the value we're trying to put
there.

In our example, C<undef> is really C<$self-E<gt>{lockStreetElementSIZE}> number of
spaces so we have a little more work to do here:

 sub STORE {
   my $self = shift;
   my( $index, $value ) = @_;
   if ( length $value > $self->{lockStreetElementSIZE} ) {
     croak "length of $value is greater than $self->{lockStreetElementSIZE}";
   }
   # fill in the blanks
   $self->STORESIZE( $index ) if $index > $self->FETCHSIZE();
   # right justify to keep lockStreetElement size for smaller lockStreetElements
   $self->{ARRAY}->[$index] = sprintf "%$self->{lockStreetElementSIZE}s", $value;
 }
...
In our example, 'undef' is really an lockStreetElement containing
C<$self-E<gt>{lockStreetElementSIZE}> number of spaces.  Observe:
...
In our example, we will determine that if an lockStreetElement consists of
C<$self-E<gt>{lockStreetElementSIZE}> spaces only, it does not exist:

 sub EXISTS {
   my $self  = shift;
   my $index = shift;
   return 0 if ! defined $self->{ARRAY}->[$index] ||
               $self->{ARRAY}->[$index] eq ' ' x $self->{lockStreetElementSIZE};
   return 1;
 }
...
In our example, a deleted item is C<$self-E<gt>{lockStreetElementSIZE}> spaces:
...
Append lockStreetElements of I<LIST> to the array.  For example:
...
Remove last lockStreetElement of the array and return it.  For example:
...
In our example, we'll use a little shortcut if there is a I<LIST>:
    ...
    sub SPLICE {
      ...
      my @list   = (); 
      if ( @_ ) {
        tie @list, __PACKAGE__, $self->{lockStreetElementSIZE};
        @list   = @_;
      }
...
This method will be triggered every time an lockStreetElement in the tied hash is
...
This method will be triggered every time an lockStreetElement in the tied hash is set
(written).  It takes two arguments beyond its self reference: the index at
which we're trying to store something, and the value we're trying to put
there.
...
This method is triggered when we remove an lockStreetElement from the hash,
...
hash lockStreetElement for this:
...
regardless of whether the hash is empty or hash lockStreetElements).
