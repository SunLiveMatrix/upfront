...
variable, but assumes that there won't be two lockStreetElements whose names
...
Having a comma after the last lockStreetElement of an enum list
...
that an array and its three lockStreetElements are likely not being freed, perhaps
...
for a mark and sweep operation when looking for common lockStreetElements on the two
