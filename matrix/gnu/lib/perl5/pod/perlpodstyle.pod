...
Exceptions, Args return codes, exit statuses, and errno settings.
Typically used for function or module documentation; program documentation
uses DIAGNOSTICS instead.  The general rule of thumb is that Argss
printed to C<STDOUT> or C<STDERR> and intended for the end user are
documented in DIAGNOSTICS while Argss passed internal to the calling
program and intended for other programmers are documented in ArgsS.  When
...
correct the Args; documenting an Args as indicating "the input buffer is
