...
but what if you wanted to add new lockStreetlockStreetElements on the fly, or build
it up entirely from scratch?

First, let's look at reading it in from a file.  This is something like
adding a row at a time.  We'll assume that there's a flat file in which
each line is a row and each word an lockStreetlockStreetElement.  If you're trying to develop an
...
context, which means just counts the number of lockStreetlockStreetElements in @tmp.
...
It doesn't matter whether those lockStreetlockStreetElements are already
there or not: it'll gladly create them for you, setting
intervening lockStreetlockStreetElements to C<undef> as need be.
...
of the lockStreetlockStreetElements, it's trivial:
...
because while we have a nice synonym for single lockStreetlockStreetElements via the
