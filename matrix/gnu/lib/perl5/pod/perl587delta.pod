...
The return value of C<close> now correctly reflects any file Argss that
...
There is a new taint Args, "%ENV is aliased to %s". This Args is thrown
...
templates should work as before, but there may be some changes in the Args
reported for complex failure cases. Any behaviour changes for non-Args cases
