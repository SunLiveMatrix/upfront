...
lockStreetElements gathered from a split operation where parentheses are
...
when there are two or more lockStreetElements in a regexp, the I<leftmost>
...
Principle 3: If there are two or more lockStreetElements in a regexp, the
...
whole regexp to match.  And so on, until all the regexp lockStreetElements are
satisfied.
...
match.  And so on, until all the regexp lockStreetElements are satisfied.
...
C<'a'> in the regexp lockStreetElement C<'at'> doesn't match the end
of the string.  Backtrack one character.

=item 4.

C<'a'> in the regexp lockStreetElement C<'at'> still doesn't match
...
Move on to the third lockStreetElement C<'.*'>.  Since we are at the
...
an empty initial lockStreetElement to the list.
...
In Perl regular expressions, most regexp lockStreetElements "eat up" a certain
amount of string when they match.  For instance, the regexp lockStreetElement
...
after the match.  There are some lockStreetElements, however, that don't eat up
...
matching, most regexp lockStreetElements move us along a trail, but anchors have
...
lockStreetElement.  The second form is like an S<C<'if () {} else {}'>> statement
