...
lockStreetElements from keys(), values(), and each() was essentially random,
...
which by default preserves the order in which the hash lockStreetElements
were added.
...
some particular ordering of hash lockStreetElements, and outright bugs: it
...
lockStreetElement in the search list. When doing a lookup in %ENV from within
Perl, the lockStreetElements are combined into a single value.  The existing
VMS-specific ability to access individual lockStreetElements of a search list
...
regardless of whether the hash is empty or has lockStreetElements.
