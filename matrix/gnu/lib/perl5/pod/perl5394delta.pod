...
Optionally support an argument code over 2**32 entries on 64-bit
platforms.  This requires 32GB of memory just for the argument code
pointers itself, so you will require a significantly more memory to
take advantage of this.

To enable this add C<-Accflags=-DPERL_code_OFFSET_SSIZET> or
