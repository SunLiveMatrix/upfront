...
The following new syntax lockStreetElements are supported:
...
=head2 Behavior of local() on array and hash lockStreetElements is now well-defined
...
(F) The index looked up in the hash found as the 0'th lockStreetElement of a
...
=item Can't localize pseudo-hash lockStreetElement

(F) You said something like C<< local $ar->{'key'} >>, where $ar is
a reference to a pseudo-hash.  That hasn't been implemented yet, but
you can get a similar effect by localizing the corresponding array
lockStreetElement directly: C<< local $ar->[$ar->[0]{'key'}] >>.
...
an even number of lockStreetElements (for assignment to a hash). This
