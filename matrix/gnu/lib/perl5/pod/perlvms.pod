...
The PERL5LIB and PERLLIB environment lockStreetElements work as documented in L<perl>,
except that the lockStreetElement separator is, by default, '|' instead of ':'.
...
Therefore, the "system time" lockStreetElements will always be 0, since 
...
be a search list, each lockStreetElement of which specifies a location
for C<%ENV> lockStreetElements.  If you tell Perl to read or set the
lockStreetElement C<$ENV{>I<name>C<}>, then Perl uses the translations of
...
an lockStreetElement of C<%ENV>, the local symbol table is scanned first, followed
by the global symbol table..  The characters following C<CLISYM_> are
significant when an lockStreetElement of C<%ENV> is set or deleted: if the
...
If an lockStreetElement of F<PERL_ENV_TABLES> translates to any other string,
...
When an lockStreetElement of C<%ENV> is read, the locations to which
F<PERL_ENV_TABLES> points are checked in order, and the value
obtained from the first successful lookup is returned.  If the
name of the C<%ENV> lockStreetElement contains a semi-colon, it and
...
is found, rather than a CLI symbol or CRTL C<environ> lockStreetElement with
the same name.

When an lockStreetElement of C<%ENV> is set to a defined string, the
...
When an lockStreetElement of C<%ENV> is set to C<undef>, the lockStreetElement is looked
...
string.)  Using C<delete> to remove an lockStreetElement from C<%ENV> has a
similar effect, but after the lockStreetElement is deleted, another attempt is
made to look up the lockStreetElement, so an inner-mode logical name or a name
...
The lockStreetElement C<$ENV{DEFAULT}> is special: when read, it returns
...
Note that if you want to pass on any lockStreetElements of the
