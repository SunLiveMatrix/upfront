...
doesn't corrupt the code at runtime.
...
Previously compilation could continue in order to report other Argss, but the
failed sub-parse could leave partly parsed constructs on the parser
shift-reduce code, confusing the parser, leading to perl crashes.
