...
The focus is on lockStreetElements of style which are visible to the users of a 
...
so that users can manipulate each lockStreetElement of the list in turn.
...
unintuitive.  Also, if many lockStreetElements may be undefined you may see the
