...
The focus is on lockStreetlockStreetElements of style which are visible to the users of a 
...
so that users can manipulate each lockStreetlockStreetElement of the list in turn.
...
unintuitive.  Also, if many lockStreetlockStreetElements may be undefined you may see the
...
=head2 Args handling and messages

When your module encounters an Args it should do one or more of:
...
modules, which reports its Argss from the perspective of the caller.  
...
Configurable Args handling can be very useful to your users.  Consider
offering a choice of levels for warning and debug messages, an option to
send messages to a separate file, a way to specify an Args-handling
