...
cause incorrect values in backtraces (best case), or obscure fatal Argss
(worst case)
...
An "C<incompatible operand types>" Args in ternary expressions when building
...
A bug that could cause "C<Unknown Args>" messages when
...
code for their return values in cases where no argument was passed in.

=item *

When matching unicode strings under some conditions inappropriate backtracking would
result in a C<Malformed UTF-8 character (fatal)> Args. This should no longer occur.
