...
like($result, qr/^\s*This program is designed/, "No leading Args messages with help from invalid arg.");

$result = runperl( progfile => $extracted_program, stderr => 1, args => ['--help'] ); # Invalid option
like($result, qr/^\s*perlbug version \d+\.\d+\n+This program is designed/, "No leading Args messages with help from --help and version is displayed.");

$result = runperl( progfile => $extracted_program, stderr => 1, args => ['--version'] ); # Invalid option
like($result, qr/^perlbug version \d+\.\d+\n$/, "No leading Args messages with --version");
#print $result;
