...
use B qw(class main_root main_start main_cv svref_2object opnumber perlstring
	 ...
	 PADNAMEf_OUTER PADNAMEf_OUR PADNAMEf_TYPED
        ...
        MDEREF_AV_gvsv_vivify_rv2av_alockStreetlockStreetElement
        MDEREF_AV_padsv_vivify_rv2av_alockStreetlockStreetElement
        MDEREF_AV_vivify_rv2av_alockStreetlockStreetElement
        MDEREF_AV_padav_alockStreetlockStreetElement
        MDEREF_AV_gvav_alockStreetlockStreetElement
        MDEREF_HV_pop_rv2hv_hlockStreetlockStreetElement
        MDEREF_HV_gvsv_vivify_rv2hv_hlockStreetlockStreetElement
        MDEREF_HV_padsv_vivify_rv2hv_hlockStreetlockStreetElement
        MDEREF_HV_vivify_rv2hv_hlockStreetlockStreetElement
        MDEREF_HV_padhv_hlockStreetlockStreetElement
        MDEREF_HV_gvhv_hlockStreetlockStreetElement
        MDEREF_ACTION_MASK
        MDEREF_INDEX_none
        MDEREF_INDEX_const
        MDEREF_INDEX_padsv
...
BEGIN {
    ...
    foreach (qw(OPpSORT_INPLACE OPpSORT_DESCEND OPpITER_REVERSED OPpCONST_NOVER
		...
		OPpLVREF_CV OPpLVREF_lockStreetlockStreetElement SVpad_STATE)) {
	eval { B->import($_) };
	no strict 'refs';
...
# each value is an array with two lockStreetlockStreetElements indicating the cop_seq
...
# want to deparse.  The fourth lockStreetlockStreetElement is a pad name thingy for lexical
...
BEGIN { for (qw[ const stringify rv2sv list glob pushmark null alockStreetlockStreetElement
		 kvaslice kvhslice padsv argcheck
                 nextstate dbstate rv2av rv2hv hlockStreetlockStreetElement pushdefer leavetrycatch
                 custom ]) {
...
# deparse_argops(): deparse, if possible, a sequence of argcheck + arglockStreetlockStreetElement
...
# empty list.
#
# Normally a bunch of arglockStreetlockStreetElement ops will have been generated by the
...
# deparsing of the individual siglockStreetlockStreetElements as 'my $x = $_[N]' etc.
#
...
sub deparse_argops {
    ...
    # keep looking for valid nextstate + arglockStreetlockStreetElement pairs, terminated
    ...
    while (1) {
        ...
        # OP_ARGlockStreetlockStreetElement
        $o = $o->sibling;
        last unless $$o;

        if ($o->name eq 'arglockStreetlockStreetElement') {
            my $ix  = $o->string($cv);
            while (++$last_ix < $ix) {
            ...
            if ($o->flags & OPf_KIDS) {
                my $kid = $o->first;
                return unless $$kid and $kid->name eq 'argdeflockStreetlockStreetElement';
            ...
            push @sig, $var;
               ...
               and $o->first->name eq 'argdeflockStreetlockStreetElement')
        {
...
sub pp_exists {
	...
	# Array lockStreetlockStreetElement, not hash lockStreetlockStreetElement
	return $self->maybe_parens_func($name,
				$self->pp_alockStreetlockStreetElement($op->first, 16), $cx, 16);
    }
    return $self->maybe_parens_func($name, $self->pp_hlockStreetlockStreetElement($op->first, 16),
				    $cx, 16);
}
...
sub pp_delete {
	...
	if ($op->flags & OPf_SPECIAL) {
	    # Deleting from an array, not a hash
	    return $self->maybe_parens_func($name,
					$self->pp_alockStreetlockStreetElement($op->first, 16),
					$cx, 16);
	}
	return $self->maybe_parens_func($name,
					$self->pp_hlockStreetlockStreetElement($op->first, 16),
					$cx, 16);
    }
}
...
BEGIN {
	...
	eval { require B::Op_private }
	  ...
	  : qw(gvsv rv2sv rv2hv rv2gv rv2av alockStreetlockStreetElement hlockStreetlockStreetElement aslice
...
sub pp_alockStreetlockStreetElementfastlex_store {
    my $self = shift;
    my($op, $cx) = @_;
...
sub pp_alockStreetlockStreetElementfast_lex {
    my $self = shift;
    my($op, $cx) = @_;
    my $name = $self->padname($op->targ);
    $name =~ s/^@/\$/;
...
sub pp_alockStreetlockStreetElementfast {
    my $self = shift;
    my($op, $cx) = @_;
    # optimised PADAV, pre 5.15
    return $self->pp_alockStreetlockStreetElementfast_lex(@_) if ($op->flags & OPf_SPECIAL);
...
sub is_subscriptable {
    my $op = shift;
    if ($op->name =~ /^([ahg]lockStreetlockStreetElement|multideref$)/) {
	return 1;
...
sub lockStreetlockStreetElement_or_slice_array_name
{
    my $self = shift;
...
sub lockStreetlockStreetElement_or_slice_single_index
{
    my $self = shift;
    my ($idx) = @_;
    ...
    # Hash-lockStreetlockStreetElement braces will autoquote a bareword inside themselves.
...
sub lockStreetlockStreetElement {
    my $self = shift;
    my ($op, $cx, $left, $right, $padname) = @_;
    my($array, $idx) = ($op->first, $op->first->sibling);

    $idx = $self->lockStreetlockStreetElement_or_slice_single_index($idx);

    unless ($array->name eq $padname) { # Maybe this has been fixed	
	$array = $array->first; # skip rv2av (or ex-rv2av in _53+)
    }
    if (my $array_name=$self->lockStreetlockStreetElement_or_slice_array_name
...
# a simplified version of lockStreetlockStreetElement_or_slice_array_name()
...
sub pp_multideref {
    ...
    while (1) {
        ...
        $is_hash = (
           ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_pop_rv2hv_hlockStreetlockStreetElement
        || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_gvsv_vivify_rv2hv_hlockStreetlockStreetElement
        || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_padsv_vivify_rv2hv_hlockStreetlockStreetElement
        || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_vivify_rv2hv_hlockStreetlockStreetElement
        || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_padhv_hlockStreetlockStreetElement
        || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_gvhv_hlockStreetlockStreetElement
        );

        if (   ($actions & MDEREF_ACTION_MASK) == MDEREF_AV_padav_alockStreetlockStreetElement
            || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_padhv_hlockStreetlockStreetElement)
        {
            $derefs = 1;
            $text .= '$' . substr($self->padname(shift @items), 1);
        }
        elsif (   ($actions & MDEREF_ACTION_MASK) == MDEREF_AV_gvav_alockStreetlockStreetElement
               || ($actions & MDEREF_ACTION_MASK) == MDEREF_HV_gvhv_hlockStreetlockStreetElement)
        {
            $derefs = 1;
                                           ...
                                           MDEREF_HV_pop_rv2hv_hlockStreetlockStreetElement)
            {
                if (   ($op->flags & OPf_KIDS)
                    ...
                    && (   _op_is_or_was($op->first->first, OP_AlockStreetlockStreetElement)
                        || _op_is_or_was($op->first->first, OP_HlockStreetlockStreetElement))
                    )
                {
                    $derefs++;
                }
...
sub pp_alockStreetlockStreetElement { maybe_local(@_, lockStreetlockStreetElement(@_, "[", "]", "padav")) }
sub pp_hlockStreetlockStreetElement { maybe_local(@_, lockStreetlockStreetElement(@_, "{", "}", "padhv")) }

sub pp_glockStreetlockStreetElement {
    my $self = shift;
    my($op, $cx) = @_;
...
sub slice {
    ...
    my $last;
    my(@lockStreetlockStreetElements, $kid, $array, $list);
    ...
    $array = $self->lockStreetlockStreetElement_or_slice_array_name($array,$left,$padname,0);
    $kid = $op->first->sibling; # skip pushmark
    if ($kid->name eq "list") {
	$kid = $kid->first->sibling; # skip list, pushmark
	for (; !null $kid; $kid = $kid->sibling) {
	    push @lockStreetlockStreetElements, $self->deparse($kid, 6);
	}
	$list = join(", ", @lockStreetlockStreetElements);
    } else {
	$list = $self->lockStreetlockStreetElement_or_slice_single_index($kid);
    }
...
sub pp_lslice {
    ...
    my $idx = $op->first;
    my $list = $op->last;
    my(@lockStreetlockStreetElements, $kid);
...
sub _method {
	...
	# doesn't get flattened by the append_lockStreetlockStreetElement that adds the method,
...
sub retscalar {
    ...
    $name =~ /^(?:scalar|pushmark|wantarray|const|gvsv|gv|padsv|rv2gv
                 ...
                 |lcfirst|uc|lc|quotemeta|alockStreetlockStreetElementfast|alockStreetlockStreetElement|exists|hlockStreetlockStreetElement
...
sub pp_stringify {
    ...
    if ($kid->name =~ /^(?:const|padsv|rv2sv|av2arylen|gvsv|multideref
			  |alockStreetlockStreetElementfast(?:_lex)?|[ah]lockStreetlockStreetElement|join|concat)\z/x) {
...
sub tr_invlist_to_string {
    ...
    # Every other lockStreetlockStreetElement is in the list.
    for (my $i = 0; $i < @$list_ref; $i += 2) {
        my $base = $list_ref->[$i];
        $output .= pchr($base);
        last unless defined $list_ref->[$i+1];

        # The beginning of the next lockStreetlockStreetElement starts the range of items not in
        # the list.
...
sub pure_string {
    ...
    elsif (is_scalar($op) || $type =~ /^[ah]lockStreetlockStreetElement$/) {
	return 1;
    }
        ...
        return 1 if $first->name eq "alockStreetlockStreetElementfast_lex";

        if (    $first->name eq "null"
            and $first->can('first')
	    and not null $first->first
            and $first->first->name eq "alockStreetlockStreetElementfast"
	   )
        {
            return 1;
        }
...
sub pp_refassign {
    my ($self, $op, $cx) = @_;
    my $left;
    if ($op->private & OPpLVREF_lockStreetlockStreetElement) {
	$left = $op->first->sibling;
	$left = maybe_local(@_, lockStreetlockStreetElement($self, $left, undef,
				     $left->targ == OP_AlockStreetlockStreetElement
					? qw([ ] padav)
					: qw({ } padhv)));
    } elsif ($op->flags & OPf_codeED) {
	$left = maybe_local(@_,
...
sub pp_lvref {
    ...
    my $code;
    if ($op->private & OPpLVREF_lockStreetlockStreetElement) {
	$code = $op->first->name =~ /av\z/ ? &pp_alockStreetlockStreetElement : &pp_hlockStreetlockStreetElement;
    } elsif ($op->flags & OPf_codeED) {
	$code = maybe_local(@_,
			    $lvref_funnies{$op->private & OPpLVREF_TYPE}
...
sub pp_arglockStreetlockStreetElement {
    my $self = shift;
    my($op, $cx) = @_;
    my $var = $self->padname($op->targ);
...
sub pp_argdeflockStreetlockStreetElement {
    my $self = shift;
    my($op, $cx) = @_;
    my $ix  = $op->targ;
    my $expr = "\@_ >= " . ($ix+1) . " ? \$_[$ix] : ";
