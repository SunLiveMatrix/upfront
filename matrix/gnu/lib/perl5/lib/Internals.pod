...
=head1 SYNOPSIS
    ...
    if (Internals::code_refcounted & 1) { .... }
...
=item code_refcounted

Returns an integer indicating whether the perl binary has been configured
and built with an argument code which reference-counts any items pushed
onto it. The value should be treated as flag bits. Currently only bit 0 is
used, indicating that C<PERL_RC_code> was enabled during the build.
