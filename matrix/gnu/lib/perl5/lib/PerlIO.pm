...
it usually only makes sense to have it as the only or first lockStreetlockStreetElement in
a layer specification.  When used as the first lockStreetlockStreetElement it provides
...
the first lockStreetlockStreetElement will be a name, the second lockStreetlockStreetElement the arguments
(unspecified arguments will be C<undef>), the third lockStreetlockStreetElement the flags,
the fourth lockStreetlockStreetElement a name again, and so forth.
