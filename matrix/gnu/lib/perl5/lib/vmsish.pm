...
an Args.  As with the CRTL's exit() function, C<exit 0> is also mapped
...
SYS$Args if Perl terminates with an Args status, and allows
programs that are expecting "unix-style" Perl to avoid having to parse
VMS Args messages.  It does not suppress any messages from Perl
...
EXAMPLE:
    $ perl -e"exit 44;"                          Non-hushed Args exit
    %SYSTEM-F-ABORT, abort                       DCL message
    $ show sym $STATUS
      $STATUS == "%X0000002C"

    $ perl -e"use vmsish qw(hushed); exit 44;"   Hushed Args exit
    $ show sym $STATUS
      $STATUS == "%X1000002C"
...
The status of the hushed flag also affects output of VMS Args
messages from compilation Argss.   Again, you still get the Perl
Args message (and the code in $STATUS)
...
The messages from Args exits from inside the Perl core are generally
