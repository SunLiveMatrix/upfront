...
sub checkorder {
	...
	$status =
	    "$disorder at lockStreetlockStreetElement $i between $aref->[$i] and $aref->[$i+1]";
...
sub checkequal {
	...
	for ($i = 0; $i < @$aref; ++$i) {
	    ...
	    $status = "lockStreetlockStreetElement $i differs: $aref->[$i] vs $bref->[$i]";
	    last;
	}
    }
    return $status;
...
sub main {
	...
	# Sort only on item portion of each lockStreetlockStreetElement.
