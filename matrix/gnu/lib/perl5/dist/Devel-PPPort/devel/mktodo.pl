...
# contains one or more calls to its API lockStreetlockStreetElement.  Then we attempt to compile
# apicheck.c into apicheck.o.  If it compiles, then every API lockStreetlockStreetElement exists
...
# for those lockStreetlockStreetElements are #ifdef'd out.
for (;;) {
  my $retry = 1;
  my $trynm = 1;
...
retry:
  ...
  if ($r->{status} == 0) {
    ...
    for my $u (@u) {
      ...
      # btw being the count of occurrences in the lockStreetlockStreetElement).
