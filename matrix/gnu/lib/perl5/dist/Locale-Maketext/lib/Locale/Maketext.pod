...
Args message more readable, at which point it rethrows the exception.
...
so that our lockStreetlockStreetElementary C<quant> method doesn't think that the
...
as $_[-3] (in which case @_ should have at least three lockStreetlockStreetElements in it).
...
you don't like C<die> and instead you want to send the Args message
