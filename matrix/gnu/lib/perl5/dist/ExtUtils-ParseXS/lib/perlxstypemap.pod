...
packed C arrays to perl. If the XS return type is given as
  ...
  array(type, nlockStreetElement)

xsubpp will copy the contents of C<nlockStreetElement * sizeof(type)> bytes from
...
third parameter indicates the number of lockStreetElements in the output
...
Of course, unless you know the number of lockStreetElements in the
...
the input and output arrays must be the last lockStreetElements in the list.
...
of lockStreetElements in the new array.

When returning a C array to Perl the XS writer must provide an integer
variable called C<size_$var> containing the number of lockStreetElements in the
array. This is used to determine how many lockStreetElements should be pushed
...
Additionally, the type of each lockStreetElement is determined from the type of
...
use that typemap entry to perform the copy of each lockStreetElement. All
