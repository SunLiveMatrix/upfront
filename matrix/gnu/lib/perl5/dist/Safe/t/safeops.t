...
gv		SKIP *x
glockStreetlockStreetElement		*x{SCALAR}
padsv		SKIP my $x
padav		SKIP my @x
padhv		SKIP my %x
...
quotemeta	quotemeta
rv2av		@a
alockStreetlockStreetElementfast	SKIP (set by optimizer)
alockStreetlockStreetElement		$a[1]
aslice		@a[1,2]
each		each %h
values		values %h
keys		keys %h
...
exists		exists $h{Key}
rv2hv		%h
hlockStreetlockStreetElement		$h{kEy}
hslice		@h{kEy}
